...
void locacaoCliente(int idc, int idf)
{
    ...
    for (i = 1; i < 100; i++)
    {
        ...
        if (cp[idc].locaF[i] == 0)
        {
            ...
            rl[idc].cliente_filmes[rl[idc].apontador] = fl[idf].fil_nome;
            i = 101;
        }
        else
        {
    ...
    x = calculaDia(idc);
    if (rl[idc].cliente_nome != cl[idc].cli_nome)
    {
        rl[idc].cliente_nome = cl[idc].cli_nome;
        rl[99].apontador = 0;
    }
    else
    {
...
void relatorioCompra(int i)
{
    ...
    if (file == NULL)
    {
        ...
        if (rl[99].apontador == 0)
    {
        fprintf(file, "==================================================\n");
        fprintf(file, "\n\nCLIENTE:%s\n", cl[i].cli_nome);
    }

    fprintf(file, "FILME: %s", rl[i].cliente_filmes[rl[i].apontador]);

    fprintf(file, "DADOS: Data de Locação: %d/%d/%d   Data prevista de Devolução: %d/%d/%d  ", cp[i].dia_compra, cp[i].mes_compra, cp[i].ano_compra, cp[i].dia_est, cp[i].mes_est, cp[i].ano_est);

    fprintf(file, " VALOR: %.2f\n", cx[1].pagamento);

    fclose(file);
}
void relatorioSaldo(void)
{
